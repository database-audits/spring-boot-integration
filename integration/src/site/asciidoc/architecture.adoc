...
`DataSource`/`EntityManagerFactory` and registers its assertions as beans. An application with several
...
audit, its `*AuditAssertion` plus the `DatabaseAuditTestConfiguration` bean that exposes it) must change in
lockstep — a compile failure against core is the intended signal.
