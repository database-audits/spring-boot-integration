...
can't use that by-type resolution: it is ambiguous. The stock config is gated on a condition that makes it *back
off* there — so importing it from your test base class is a harmless no-op rather than an "expected single matching
bean but found 2" failure. Audit each datasource *by name* instead, one target per generated suite.
...
stock config, but by name. Name it after the datasource, e.g. for an `Aurora` datasource:
...
public class DatabaseAuditAuroraTestConfiguration {
    ...
    // ...one @Bean per audit assertion, each delegating to the suite...
}
----

Each audit IT `@Import`s that config and injects the assertion bean, exactly like the single-datasource suite — no
`@Primary`, and nothing injected by bare type across datasources, so the "found 2" ambiguity cannot arise:
...
`-DdataSourceName=Aurora -DdataSourceBeanName=auroraDataSource
-DentityManagerFactoryBeanName=auroraEntityManagerFactory`. To audit several datasources, generate once per
