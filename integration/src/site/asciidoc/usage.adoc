...
`EntityManagerFactory`. An application with several datasources audits each with its own
`io.github.databaseaudits.spring.boot.DatabaseAuditSuite`: a plain factory that wires the whole audit graph for a
given `(DataSource, EntityManagerFactory)` pair and exposes the same `DatabaseAuditAssertions` facade.

Keep importing `DatabaseAuditTestConfiguration` for the primary (`@Primary`) datasource, and add one
`@TestConfiguration` per additional datasource that builds a suite from its `@Qualifier`'d beans. Name each after
the datasource, mirroring `DatabaseAuditTestConfiguration` with the datasource name inserted — e.g. for an
`Aurora` datasource, `DatabaseAuditAuroraTestConfiguration`:
...
public class DatabaseAuditAuroraTestConfiguration {
    ...
    @Bean
    DatabaseAuditAssertions auroraDatabaseAuditAssertions(
        ...
        return new DatabaseAuditSuite(dataSource, entityManagerFactory, auroraSqlCapturer).assertions();
    }
}
----

Inject the named facade and assert as usual:

[source,java]
----
@Autowired
@Qualifier("auroraDatabaseAuditAssertions")
private DatabaseAuditAssertions auroraAudits;

@Test
void auroraSchemaIsClean() {
    auroraAudits.assertCatalogClean("aurora_schema");
}
----

The archetype generates one such pair per name — `DatabaseAudit<Name>TestConfiguration` plus a `@Disabled`
`DatabaseAudit<Name>IT` — when you pass `-DdataSourceNames=Aurora,Reporting`. Fill in the `@Qualifier` bean names
and schema in each, then remove `@Disabled`.
...
* *Runtime audits* (PostgreSQL-only) additionally need that datasource's statements routed into this suite's
  ...
  call only `assertCatalogClean` and `assertJpaClean` on that datasource's suite.
